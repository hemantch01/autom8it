...
interface BaseNodeProps extends HTMLAttributes<HTMLDivElement>,ComponentProps<"div"> {
  status?:NodeStatus;
}
export function BaseNode({ className,status, ...props }: BaseNodeProps) {
  return (
    ...
    >
      ...
      {status==='success'&&(
        <CheckCircle2Icon className="absolute right-0.5 bottom-0.5 size-2 text-greed-700 stroke-3"/>
      )}
      {status==='loading'&&(
